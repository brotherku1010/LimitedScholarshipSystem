...
:check_local
:: 2. Check if local portable Node.js exists
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://nodejs.org/dist/v20.12.2/node-v20.12.2-win-x64.zip' -OutFile 'node.zip'"
...
:: Move to node-portable and clean up zip
move node-temp\node-v20.12.2-win-x64 node-portable >nul
del node.zip >nul
rd /s /q node-temp >nul
...
echo.
if exist node.zip del node.zip
if exist node-temp rd /s /q node-temp
pause
exit /b
